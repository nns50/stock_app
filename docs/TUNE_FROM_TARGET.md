...
| 2026-09-15 | **Give-back guard OFF**: `giveBackArmPct` 2 → **null**, `giveBackFloorPct` 1 → **null**. Goal unchanged at **3%**; every other net unchanged (drawdown halt 7.5%, step-down 2/50%, one entry per symbol per session, stagnation exit 60 min) | The edge-leak scan's `One winner arms the give-back guard` finding, raised once the trial sizing was live, and the operator's decision after the arithmetic was put to them | **The band is narrower than one trade.** At 2.5% risk with a 1R target each trade moves the day ±2.5 points, while arm (+2%) to floor (+1%) is one point wide — so a win then a loss lands the day at 0%, under the floor, and halts the book after two trades. In R: the goal is 1.2R tall, the arm sits at 0.8R, the floor at 0.4R, the step is 1.00R. No valid band exists, because the route requires floor ≥ 0 and arm < goal. On the live record — 75 trades, 12 sessions, **47% win rate**, median 6 trades a session — win-then-loss opened **2 of 10** sessions, and the sessions truncated are the busiest (12, 11, 11, 9 trades). The pre-trial config did NOT have this problem: at 1.25% risk with a 2R target a winner moved the day +2.5% and a loser only −1.25%, so win-then-loss left the day at +1.25%, above the floor, and it took a win and TWO losses to fire — the 2026-09-12 change broke the band by doubling risk and halving the target at once. `dayProtectiveStopEnabled` is already false, so nothing else rode on the guard | Restores the flow the goal depends on most: roughly one session in five is no longer capped at two trades. Costs the give-back protection itself, which at a 1.2R goal was nearly redundant with the bank halt anyway — the bank fires at 1.2R, only 0.2R above where a single winner lands | **Revisit at the 10-session review.** If the sizing reverts to 1.25% / 2R, the 2/1 band is coherent again and the guard goes back ON with it. If 2.5% is KEPT, the guard cannot work at this goal height and a give-back rule would need a floor below zero (a band at least one trade-step wide) — a code change, not a lever |
| 2026-09-15 (second) | **No change. Recorded so the review can read sessions 3 and 4 correctly**: the account took a $30,000 deposit that settles Friday 2026-09-18, so the loop trades those two sessions against buying power far below its equity | The operator's decision, after the arithmetic was put to them: leave the code alone rather than ship a sizing-denominator change overnight to fix a two-day condition | **Net liquidation $30,462.81, deployable $4,474.11** (market value $451.50 + overnight buying power $4,022.61; day BP $8,045.22). Every %-of-equity number derives from net liquidation, so at the effective 1.43% risk in force (step-down x0.50 active) the loop targets ~$436 a trade — a $21,781 notional at a 2% stop, 37% fundable, clamped to about a third; a 1% stop asks $43,562, funds 18%, and is skipped under the 25% floor. The 3% goal reads ~$913 against a book that can deploy $4.5k. Nothing is unsafe — the sizer and the broker both refuse rather than overshoot — and the loss the account took was a manual SPY 0DTE put, not the loop (strategy day **+$42.84**, +0.13%) | Sessions 3 and 4 of the ten-session review will be SMALL for a reason that has nothing to do with the sizing under test. Every clamp is journaled with both numbers (`sized down to X of Y shares to fit $Z of buying power — risking $A instead of the intended $B`), so those sessions are identifiable, not lost | **Read this row at the 10-session review before reading the mean day.** If sessions 3 and 4 are materially below the rest, check their `buying_power_sizing` lines before concluding the trial underperformed. `deployableEquityUsd` — deriving sizing, caps and the goal from capital the book can actually use — is deferred to a proper PR for the NEXT deposit, unhurried and with real tests, rather than shipped against a two-day deadline |
| 2026-09-15 (third) | **Day-protective stop ON**: `dayProtectiveStopEnabled` false → **true**, `dayProtectiveStopFloorPct` null → **1.5**. Goal unchanged at 3%; the give-back guard stays OFF; nothing else moves | The operator's decision, after the upsides and downsides were laid out — asked directly how the account should handle reaching 3% | **Banking the day halts ENTRIES; it never sells.** A realized 3% can still be handed back by whatever is open, and a *marked* 3% (positions up, nothing closed) the goal cannot see at all. This rule is the narrow answer to the first case: per open position it asks "can THIS trade still cost me the day?" and, only when the current stop would breach the floor, moves the stop to exactly where a stop-out lands the day at +1.5% — never further, never looser, and never closer than a quarter of the trade's original risk (below that it declines and leaves the stop alone). At the $33,694.39 baseline the floor is **$505.42**, half the $1,010.83 goal. It had never run in production: it was gated on the give-back guard being armed and took its floor from the guard, both nulled this morning, so it went dark with a rule it has nothing to do with (fixed in #619) | Fewer "we were up and gave it back" days, paid for in slightly smaller average winners: a tighter stop is hit more often, so some trade that would have dipped and run to target now scratches. **That cost is unmeasured** — zero firings on the record, because the rule has never run. Known limit, written into the code: it is PER POSITION and the headroom is not shared, so two positions both stopping out can still take the day to twice the distance below the floor. It bounds one trade's give-back, not the book's | **Pre-committed reading, trigger `trig_012MooAxggSKCxVfFKYS4hzd`, nightly to 2026-09-30.** Every `live_stop_ratcheted` with `kind: 'day_protective'`, and for each that then stopped out, what the stock did afterwards. Verdict at 10 firings or 2026-09-30: *stock usually kept going* → the tightening costs winners, propose a lower floor or the exit-reason-aware re-entry cooldown (the operator's 2026-09-15 idea, held back for want of evidence — re-entering after a protective stop re-creates the risk into a SMALLER cushion, and the round-2 record is −$185 over 20); *usually kept falling* → keep it; *rarely fires* → say so, the floor may be too high to matter. **NOT on the table: halting entries when it fires.** The operator was shown that the floor does not survive the next trade — at 2.5% risk one new full-size loser is ~$761 against a $505.42 floor — and decided on 2026-09-15 that the loop SHOULD keep trading toward 3% anyway. So this rule bounds what an already-open trade can take back and nothing more; the session's only real floor is the -7.5% drawdown halt. Read as a decision, not an oversight |
| 2026-09-18 | **Aggregate open-risk budget widened by direct instruction**: `maxAggregateOpenRiskPct` 7.5 → **12.5**. Goal unchanged at 3%; risk per trade, slots, the halt and every other net unchanged | The operator's decision ("ok widen it to 12.5%"), after the 11:28 ET read showed the shared budget refusing every options entry while the stock book was full | At 2.5% risk × 3 stock slots the 7.5% budget was full whenever the stock book was, so the options sleeve (2 slots) could open only into an empty stock slot: 5 `max_aggregate_open_risk` refusals between 10:01 and 10:10 ET on 2026-09-18, on top of the two options-cap bugs fixed in #629 (the daily order count and the exposure ceiling both fed an account-wide figure against an options-only cap) | Both sleeves can run at once: 12.5% = 2.5% × the five slots across the two sleeves. With every slot filled the book carries up to 12.5% of equity at risk against a 7.5% halt that acts on REALIZED loss, so a day that stops everything out at once can end near −12.5%, against −7.5% before | **Exposure, taken knowingly; read at the 10-session review alongside the 2026-09-12 row.** Pre-committed check: the first `live_options_order_placed` while three stock positions are open. If the worst day of the review window exceeds the halt by more than one trade's risk, this is the first number to put back |
| 2026-09-24 | **No change. Recorded so the review reads the 2026-09-12 row's exit evidence correctly** | Found while re-reading the exit replays (`docs/AUTOTRADING_SPEC.md`, 2026-09-24 (fifth)) | The 2026-09-12 row argued "exit geometry is not the lever (twelve stop/target shapes all replay `inside_noise`)". Those comparisons walked each path only to the trade's actual exit, so a shape that holds longer (a 2R target, a later or no stagnation scratch) could not read as better. Uncensored on the production copy (live, 104 same-session trades, honest fills): a 2R target +0.025R (−0.031…+0.074), a 90-minute scratch +0.028R (−0.006…+0.063), no scratch +0.056R (−0.005…+0.105); paper, 128 trades: a 2R target +0.038R. All inside the noise, all leaning toward holding longer | None: the 1R target and the 60-minute scratch stay, since nothing here clears its interval | **At the 10-session review, re-read the exit shapes on the uncensored replay** (on honest fills once that change ships) before treating "exit geometry is not the lever" as settled |
| 2026-09-26 | **No change. Series boundary: the exit comparisons read honest fills** | The replay-honesty follow-up (`docs/AUTOTRADING_SPEC.md`, 2026-09-26 (eighth)) | `/exit-replay` (its replay and its `c`-prefixed comparison) and `/exit-tune-validation` replayed every arm under `touch` fills: a stop filled at its price through a gap, the breakeven and trail armed on a bar's extreme, a target filled on a touch. That optimism does not cancel between two shapes: on the live book, no stagnation scratch read +0.039R under `touch` against +0.056R `honest` (2026-09-24). Both routes now replay `honest` and say so (`fills`); `?fills=touch` reproduces a reading taken before this date | None: no setting or switch reads these routes; only the numbers a person reads move | The 10-session review's re-read of the exit shapes (the 2026-09-24 row) is the first on honest, uncensored paths. Set it only beside readings that carry `fills: 'honest'`, or re-read the old ones with `?fills=touch` |
