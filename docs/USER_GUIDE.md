...
- **Tune from target daily gain** (2026-07-23, collapsed by default, right below Core
  ...
  **max daily drawdown** halt, the losing-streak **step-down**, one entry per symbol per
  ...
  > P&L passes the floor it journals one **`day_protective_armed`** row, under **Live** in
  > Recent activity: the day's gain, the floor, the dollars above it, and its verdict on
  > each open position.
  >
  ...
  **Allow naked short** guardrail under Live trading, below — with Short or Both
  ...
  live shorts — until then it left no row at all. Since 2026-09-26 it is journaled once
...
- **Live trading** — configure and arm the loop to place **real** orders through Webull.
  ...
  Nested under it, **Short only on a broadly red market** (2026-09-26, on by default)
...
because the loop is the only caller that is always flat by the bell, so it is
  ...
  **An estimate that was right is marked confirmed, and one left at the quote says why.**
  ...
  Webull's order lists can lag a fill by a few minutes. So an entry placed within the last
  seven days that neither list shows yet is not given up on: the row says
...
**One exception: a tripwire** (2026-09-26). "Turn live shorts off" acts the first time it
...
Since 2026-09-26 the reading goes on with the **red-market bar**: the shorts declined on a
...
**Since 2026-09-26 the rule proposes on the red-market bar, not the old one.** Live shorts
...
**A losing short book turns itself off** (2026-09-26). While live shorts are on, "Turn
...
- **Sync positions from Webull.** In **Settings → Webull**, preview your open Webull
  ...
  **Since 2026-09-25 the app books those closes at your fill itself**, for the account the
  ...
  **`hand_exit_correction_skipped`** says so once, listing what the history held. This also
  ...
  own table, so the sync imports each of its contracts here too, and until 2026-09-25 it
