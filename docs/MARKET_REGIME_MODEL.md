...
- **Consumers (2026-09-08).** With the auto-trade config's `mlRegimeEnabled` on (off by
  ...
  constant in R; `docs/TUNE_FROM_TARGET.md` §6c), and `mlRegimeHighVolMinSignalScore`
  raises the live conviction bar in that regime (`entryScoreGate.ts`, live only); the
  stamped factor feeds the counterfactual MFE ledger (`regimeTightenLedger.ts`,
  `GET /api/journal/regime-tighten`), whose pre-committed reading after 30 tightened trades
  decides whether the tighten stays. The
