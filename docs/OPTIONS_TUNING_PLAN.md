# Options tuning plan — how the daily read turns into changes

Companion to `docs/SHORT_DATED_OPTIONS_SPEC.md`. That document says what was
built and why. This one says **how it gets improved**, and — more importantly —
**when it does not**.

Written 2026-08-27, the day short-dated options went live on the paper book.
Every parameter in the ladder came from a Black-Scholes model at a single
volatility on a single hypothetical underlying. None has been checked against a
real trade. This plan exists so that changes to those numbers come from
evidence rather than from the most recent day's mood.

---

## The failure this is built to prevent

A daily check-in with no memory has exactly two failure modes, and they look
like opposites:

1. **It never changes anything.** Day 1: "sample too small." Day 2: "sample too
   small." Day 20: "sample too small." Each read is honest and each read is
   useless, because nothing accumulates.
2. **It changes something every day.** Each day's noise gets a parameter tweak,
   the tweaks interact, and after two weeks the system is worse than the model
   it started from — with no way to tell which change did it.

The defence against both is the same: **decide the rules before seeing the
data.** Everything below is pre-committed. When a threshold is met, the change
is made and logged. When it is not, the read says "no change" and moves on —
and that is a result, not a failure to produce one.

---

## Layer before parameter

The single most common tuning mistake is fixing the wrong layer. These must be
diagnosed **in order**, and a layer is only worth tuning once every layer above
it is healthy:

| Layer | Question | If broken, nothing below it means anything |
|---|---|---|
| **0. Funnel** | Does it place a trade at all? | Zero entries → exit stats are empty. Tuning the ladder here is tuning nothing. |
| **1. Signal** | Do entries get followed by movement? | If entries go nowhere, that is the *entry criteria*, not the exit. Loosening stagnation just holds losers longer. |
| **2. Ladder** | Do exits capture what movement there is? | Only meaningful once 0 and 1 are healthy. |
| **3. Edge** | Is any of this profitable? | The honest question. Answered at review, not daily. |

**Layer 0 is where this starts, and it is not a formality.** On 2026-08-26 live
options generated 14 signals and placed **zero** orders — every one died on
`risk budget is too small to size even one contract at this premium`. That is
the entire reason short-dated exists. Whether that reason has disappeared is
the first thing the daily read checks, every day, before anything else.

---

## Pre-committed decision rules

Each rule names its trigger, its minimum sample, and its response. **No
parameter moves without one of these firing.**

### Layer 0 — funnel

| # | Trigger | Min sample | Response |
|---|---|---|---|
| F1 | ≥3 consecutive sessions with 0 options entries, same dominant block reason | 3 sessions | Diagnose that reason per F2–F5. Do not touch the ladder. |
| F2 | Dominant reason is **risk budget** | 3 sessions | **No loosening.** Short-dated was the fix for this; if it still dominates, options are structurally unreachable at this account size. Report that and stop. Raising `riskPerTradePct` to reach an instrument is backwards. |
| F3 | Dominant reason is **liquidity** (`optionsMaxSpreadPct` 8, `optionsMinOpenInterest` 100, `optionsMinVolume` 10) | 3 sessions | **Measure before changing.** These were set for 7–21 DTE contracts; 0–2 DTE spreads behave differently. Sample the actual spread/OI/volume distribution on 0–2 DTE contracts across the universe, then set the gate from the observed distribution. Never widen a liquidity gate to hit a trade count. |
| F4 | Dominant reason is **DTE window** | 3 sessions | Likely structural: daily expiries exist for index ETFs and a handful of mega-caps, not for most of a 500-name universe. Report the share of the universe that *has* a 0–2 DTE chain. If it is small, the honest fix is a narrower universe for options, not a wider DTE band. |
| F5 | Dominant reason is **IV/RV** (`optionsMaxIvRvRatio` 1.0) | 5 sessions | 0DTE IV is structurally elevated into the close. If this blocks most candidates, report the observed IV/RV distribution before proposing a number. |
| F6 | `short_dated_entry_window_closed` on ≥50% of sessions | 5 sessions | The 210m cutoff is eating the day. Check *when* signals arrive: if they cluster after 12:30 ET, the problem is signal timing, not the cutoff. |
| F8 | Options entries refused on `max_concurrent_positions` while the options book itself has room | 1 session | The shared slot budget is starving the evidence track. Give options its own slots via `optionsMaxConcurrentPositions` — do **not** raise `maxConcurrentPositions`, which is shared with live equity and would let real money open another position to fix a paper-book problem. Found 2026-08-27: 184 options signals, zero orders, every one refused "2 open vs cap 2" while equity held both slots all session. |
| F7 | "max 1 at a time" refuses ≥5 candidates in a week | 1 week | Note it. Do **not** raise the cap — §D6 of the spec is explicit that two 0DTE positions can go to zero on one adverse move. Revisit only after ≥30 closed trades show the exits work. |

### Layer 1 — signal

| # | Trigger | Min sample | Response |
|---|---|---|---|
| S1 | `stagnation` ≥40% of exits | 10 closed trades | The entry signal is not producing movement. **Do not loosen stagnation.** Report which entry criteria the stagnant names shared. |
| S2 | Median underlying move at exit < 0.3% in either direction | 10 closed trades | Same conclusion as S1 by a different route: the names being picked do not move enough for this instrument. |

### Layer 2 — the ladder

**No ladder parameter moves before 15 closed short-dated trades.** Below that,
the read reports the distribution and says so.

| # | Trigger | Min sample | Response |
|---|---|---|---|
| L1 | `disaster_stop` fires **at all** | 1 | Immediate investigation, sample size irrelevant. It means something outran the underlying stop between ticks. Report the case in full — entry, tick timeline, underlying path. |
| L2 | `take_profit` never fires while `give_back` does | 10 closed trades | The 60% target is above what these contracts reach. Lower it to the **observed median peak gain**, not to a guess. |
| L3 | `hard_time` ≥50% of exits | 10 closed trades | Positions open too late or the thesis is too slow. **Tighten the entry cutoff, never loosen the 14:00 clock** — the clock is the one rule whose cost is arithmetic. |
| L4 | `underlying_stop` ≥50% of exits **and** median exit is a loss | 15 closed trades | 0.5% may be inside the noise band for the names being traded. Compare against each name's own ATR% before proposing a number. |
| L5 | `give_back` fires and the median outcome is worse than holding to the 60% target would have been | 15 closed trades | The trail is too tight. Raise `optionsGiveBackPct` toward 60–65 — one step, then re-measure. **How to evaluate it:** compare each `give_back` exit's `peakGainPct` against `optionsTakeProfitPct`. A peak BELOW the target could never have exited at the target however long it was held, so it is not evidence the trail was tight; only peaks ABOVE it are. Post-exit prices are never needed. `peakGainPct` was computed by the ladder from the start but not journaled until 2026-09-09, so **the first 15 closed trades cannot be evaluated** — L5's clock effectively starts from that date. |
| L6 | Any single rule accounts for ≥70% of exits | 15 closed trades | The ladder has collapsed to one rule; the others are dead weight. Report which, and why the rest never get a turn. |

### The always-check

| # | Trigger | Response |
|---|---|---|
| A1 | Any options position open after the close | **Lead the report with it.** The ladder failed. Everything else in the report is secondary. Since 2026-09-12 the expiry sweep settles an expired 0DTE the same evening, so an *expired* contract no longer lingers overnight — the evidence for this rule is now `live_options_expired_worthless` with `expiration` equal to the session date, a `live_options_expired_needs_review`, or an open position with no expiry at all. `live_options_exit_reprice_deferred` (reason `daily_cap`) is the near-miss: the close was chased twenty times and still could not be sold. |

---

## Change budget

- **At most one parameter change per week**, so an effect is attributable to a
  cause. Two changes in a week make both uninterpretable.
- **Never two changes on the same day**, regardless of how many rules fire. If
  two fire, take the one on the **lowest layer** — a funnel fix precedes a
  ladder fix, always.
- **Every change is logged below** with its trigger, evidence, and the metric it
  was expected to move.
- **Rollback rule:** if a change has not moved the metric it targeted within 5
  sessions, revert it. A change that does nothing is not neutral — it is noise
  in the next analysis.

---

## What the daily read actually queries

The journal writes roughly 1000 rows every 8 hours overnight and far more
during market hours, and the row endpoint caps at 1000 — so **an unfiltered
read cannot see yesterday.** Measured 2026-08-27: 1000 unfiltered rows spanned
7.9 hours.

Narrowing by action helps, but **only for low-volume actions.** Also measured
that day, at `limit=1000`:

| Query | Reach |
|---|---|
| unfiltered | 7.9h — cannot see yesterday |
| `actions=blocked` | 3.5h — still capped, worse in wall-clock (and this was both funnels at once) |
| `actions=options_signal_generated,no_options_signal` | 5.4h — still capped |
| `actions=short_dated_options_exit` | uncapped (≤4/day, so years) |

So the two reads split:

**Layer 2 (the ladder) reads rows.** `short_dated_options_exit` fires at most
once per closed position, against a max of one concurrent position and a 4/day
order cap — a handful a day. The full history fits well under the cap, and the
rows carry the `rule`, `premiumGainPct` and `underlyingMovePct` the L-rules
need:

```
GET /api/autotrade/events?actions=short_dated_options_exit&since=<epoch_ms>&limit=1000
```

**Layer 0 (the funnel) reads counts, not rows.** `blocked` and the signal
actions fire hundreds of times an hour during market hours, so no amount of
paging reaches a two-week window. Use the summary endpoint, which counts by ET
calendar date and never materialises the JSON `detail` blob that makes a row
heavy:

```
GET /api/autotrade/events/summary?actions=<names>&since=<epoch_ms>
→ { summary: [{ date: 'YYYY-MM-DD', action, count }, ...] }
```

| Purpose | Endpoint | actions |
|---|---|---|
| Exit rule distribution (L1–L6) | `/events` | `short_dated_options_exit` |
| Entry funnel (F1–F5) | `/events/summary` | `options_signal_generated,no_options_signal` |
| Risk-check blocks (F2–F5) | `/events/summary` | **`options_blocked`** |
| Entry gates (F6–F7) | `/events/summary` | `short_dated_entry_window_closed` |

**`options_blocked`, not `blocked` (2026-09-09, task #53).** Until that date the
options risk check journaled `action: 'blocked'` — byte-identical to the equity
check — so this row returned the **union of both funnels**. On 2026-09-04 the
options read and the equity read both showed `blocked = 2,015`, because they
were the same rows, and F2–F5 could not be evaluated from counts at all. Same
class as the F7 fix: a gate whose event is indistinguishable from another
gate's is not measurable. The options funnel now writes `options_blocked` /
`options_passed`; `blocked` is the equity funnel alone. **Rows written before
2026-09-09 are still under `blocked` and cannot be split** — for a window
spanning that date, query both and treat the earlier part as an upper bound.
The LIVE options path was never affected (`live_options_entry_blocked`,
`live_options_risk_blocked`).

`since` should be the epoch ms of **2026-08-27**, when short-dated was
enabled, so every read covers the whole life of the feature rather than one
day.

One thing the summary cannot give you: the *reason* inside an `options_blocked`
event's `detail`. When F1 fires and the dominant reason has to be identified,
pull the rows for a single recent session (a narrow `since`) and read the detail
there — the summary establishes *that* blocks dominate, the rows establish
*why*.

## Data-quality notes

Facts about the ACCOUNT that are not facts about the STRATEGY. Any analysis
reading `accountEquityUsd`, the daily-target gain %, or anything sized off
them has to know about these; the trade-level record does not, and the
distinction is the whole point of the section.

| Date | What happened | What it contaminates | What stays clean |
|---|---|---|---|
| 2026-08-27 | **+$5,000 deposited**, and **−$2.2k of manual (non-autotrade) trading**, on a day that also carried a hand-taken SPY 0DTE call | `accountEquityUsd` ($2,450 → $5,142 in one step), the daily-target gain % (read **+130.71%**, none of it trading), and every %-of-equity size derived from them. The equity series has a **discontinuity here — do not difference across it** | Autotrade's own P&L, which is computed from **tagged positions**, not from equity. Its real number for the day is **−$8.32** on one SMCI round trip. Manual fills are untagged and never enter it, so the trade-level record the 2026-09-05 review depends on is unaffected |
| 2026-09-21 / 09-22 | Four live options closes were booked by the **broker-position sync**, not by the order reconcile: TSLA #11, INTC #10 (09-21), GOOGL #13, AAPL #12 (09-22). Each was booked at an **estimate** from the delayed chain, with exit reason `manual` (the `live_options_position_closed` row carries `via: 'broker_sync'`). Both days the operator engaged the kill switch and closed positions by hand in Webull. Until 2026-09-23 an app close the order lists missed was booked the same way | Those four rows' exit prices and P&L (GOOGL's estimate equals its entry exactly, $0.00), and any ladder attribution by exit reason that includes them | Rows closed through the reconcile (no `via`), which carry the broker's fill price and the rule that fired. From 2026-09-23 the reconcile asks Order Detail about a close neither list shows, before the sync runs (AUTOTRADING_SPEC "2026-09-23 (fourth)") |
| 2026-08-27 (fixed) | The same deposit, as a **recurring** hazard | Nothing further — from this date the loop detects an external cash flow and moves the day's baseline by it instead of counting it as gain (`daily_baseline_rebased`), so a later deposit neither banks the day nor distorts the %-of-equity caps | The 2026-08-27 session itself is still contaminated as described above; the fix is not retroactive, so treat that date's equity-derived figures as unusable regardless |

The general rule this establishes: **equity-derived series are account facts and
carry deposits, withdrawals and manual trading; position-derived series are
strategy facts and do not.** When the two disagree, the position-derived one is
the one that says anything about whether this works.

---

## Decision log

Append-only. Every parameter change to the options path goes here, whether it
came from a rule above or from a direct instruction. A change not in this table
did not happen for the purposes of the next analysis.

| Date | Change | Trigger | Evidence | Expected effect | Outcome |
|---|---|---|---|---|---|
| 2026-09-12 | **No parameter change.** Mechanism: options closes priced at the real-time **bid** (OPRA snapshot, then the chain's bid, then 5% through the mark); a working close **chased every tick** instead of only when a clock rule fires; a sub-tick price floored at one tick instead of refused; the expiry sweep settling a 0DTE the same evening | Rule A1 fired — HOOD 260911C116 was open after the close on 2026-09-11 | One contract at $0.90, 10:16 ET. `take_profit` at +64% placed 1.40 against a 1.47 mark and never filled; nothing re-examined it until `give_back` at 11:15 (−43%); the 14:00 `hard_time` replacement computed `roundOptionPrice(0.03 × 0.95) = 0` and was refused "below the $0.05 option tick" every tick to 17:05; expired worthless. +$58 became −$90. The ladder decided correctly three times | Decisions the ladder already makes correctly start reaching the account. No change to any rule LEVEL — take-profit stays 60, the give-back arm/fraction stay 40/50, the underlying stop stays 0.5% | — |
| 2026-09-12 | `optionsTakeProfitPct` **kept at 60** (a rise to 100 was proposed and withdrawn) | Considered as part of the 3%-goal push | Re-scoring the paper book's 20 short-dated trades (avg **+17% of premium**, 12 winners; 5 take-profits at +64…+88%, 5 give-backs at +9…+28% off peaks of 42–64%) against lower and higher targets: 40% reads ≈ +13%, 30% ≈ +9%, and a higher target is what the give-back trail already protects against. The level is not the problem; the fill was | Nothing changes. Recorded so the next reader does not re-propose it | — |
| 2026-08-27 | `shortDatedOptionsEnabled` false → **true**; DTE band 7–21 → **0–2**; `liveOptionsEnabled` true → **false** | Roll-out step 3 of the spec | 2026-08-26: 14 live options signals, 0 orders, all on risk budget vs $241–340 contracts against a ~$44 budget | Options become affordable; paper starts producing exit-rule data | — |
| 2026-08-27 | `optionsTakeProfitPct` 75 → **60** | Spec §D3 | Modelled: reachable on a ~+0.8% underlying move before noon; 75 was not | `take_profit` fires at all, rather than every winner ending on `give_back` or `hard_time` | — |
| 2026-08-27 | `optionsMaxConcurrentPositions` 0 → **1** | F8 | 184 options signals that session, **zero** orders — 930 blocks all reading "2 open vs cap 2" while paper equity (GREE, HRL) held both slots from the open | The paper options book can open a position at all, so the evidence track produces evidence | — |
| 2026-08-27 | The max-1 short-dated gate now journals `short_dated_position_already_open`; `liveOptionsMaxOrderUsd` **stays tied to the equity cap** (an option-BP bound was built, then withdrawn) | F7's own measurability, plus a follow-up to the row below | The gate returned silently, so **F7 had no event to count** — a gate throttling the book looked identical to one that never fired. Separately, option BP was **$471.41 at 16:30 and $322.36 an hour later**, against a day BP of $8,644.72 | F7 becomes measurable rather than nominal | **The option-BP bound was reverted before merge, deliberately.** These are STORED caps and `liveCapsReanchor` re-derives them from config alone, with no broker call, so it cannot see option BP. A cap bound to a figure only the tune observes — and which moved 32% in an hour — would read as hand-edited to the re-anchor and freeze out of re-anchoring: the exact trap the row below records. Bounding options **orders** by option BP belongs at use time, in `liveOptionsExecute`, where the live figure is in hand. Not done here: live options is off, so it gates nothing today |
| 2026-08-27 | `liveMaxOrderUsd` 3000 → **3871**, `liveMaxDailyLossUsd` 344 → **331**, `liveOptionsMaxDailyLossUsd` 344 → **331**, `liveCapsAnchorEquityUsd` 5352.23 → **5161.18** | Operator instruction: the per-order cap "should be auto established depending on the BP available" | The cap and the sizer disagreed by construction: the sizer asks for `riskPerTradePct / maxStopDistancePct` = 1.25/2.5 = **50% of equity ($2,580)**, the cap allowed the moderate band's **25% ($1,290)**. Every order was larger than the cap that had to approve it | Caps are re-derived from the merged formula — `max(band, sizerFloor x 1.5)` bounded by buying power (day BP $8,644.72, non-binding here) — so the cap clears the sizer floor at **1.50x** and entries stop failing on their own configuration | **Moving the anchor to current equity is the point.** Written alone, these would read as hand-edits and be frozen out of re-anchoring forever; with the anchor moved they are anchor-owned, so every future re-anchor maintains them from equity automatically. `liveOptionsMaxOrderUsd` deliberately **left at 1338** — live options is off, options orders are premium-sized (~$30–50), and option BP is only **$471.41**, so the derived $3,871 would be a backstop that cannot backstop. It stays hand-edited. Binding the stored twin to option BP was tried and reverted the same day (see the row above) — the right place for that bound is use time, not a stored cap |
| 2026-09-05 | The buying-power bound on the **equity** per-order cap is removed — `deriveDollarCaps` is now a pure function of equity and the config percentages. Funding still reaches `/tune/preview`, but only to raise a **warning** | Audit follow-up: the options-side bound was reverted on 2026-08-27 for a reason that applied just as much to the equity side, and nobody carried it across | `deriveDollarCaps` has three call sites and only ONE — the tune apply — has a broker call. `handEditedDollarCaps` and `liveCapsReanchor` work from config alone, by design. So on any day funding actually bound the cap, the tune STORED the smaller figure while the anchor check re-derived the larger one; the two disagreed, and the cap was flagged hand-edited and skipped by **every future re-anchor** | The stored caps describe intent and stay anchor-owned; funding is enforced at decision time by `fundableMaxQuantity`, where the live figure is actually in hand | **Never triggered in production** — day BP ($8,644.72) sat above the derived cap ($3,871) every time the bound was live, so it never bound and never froze anything. The old unit tests passed under the defect throughout: they asserted the arithmetic of `deriveDollarCaps` where it is COMPUTED, and the failure was a disagreement BETWEEN two callers. The regression test now drives a tune under starved BP and asserts `handEditedDollarCaps` returns empty and the next re-anchor still moves the caps |
| 2026-08-27 | Universe: **+SPY, +QQQ** (526 → 528) | Operator instruction, after a manual SPY 0DTE call carried the whole day's +11% | The loop screened 34 distinct names that session — OKTA, HRL, DG, CRWD — and **no index ETF once**. The instrument the short-dated ladder was built for is one the universe could not see | SPY/QQQ reach the options decision, where 0–2 DTE chains actually exist and are liquid | **Watch, do not tune.** There is no separate options universe: options candidates are the equity-screened set, so both must first clear a single-name volatility-breakout screen. Three plausible blocks — `minChangePct: 1` (an index rarely moves 1%), `skipped_unknown_sector` (ETFs have no GICS sector; fired 69× that day), and `optionsMaxIvRvRatio: 1` (index IV usually sits above realized). No gate was loosened to make room; the next read reports where they actually die |
| 2026-09-06 | `autoTuneMaxStepPct` 0.5 → **0.25** | Operator decision, after the Tuesday audit found auto-tune armed to raise risk and asked what the step actually controls | The field is absolute PERCENTAGE POINTS per day and the tune runs DAILY — 49 runs, one every day since 07-20. 0.5 was the shipped default (`autotradeConfig.ts:1207`), never a number chosen for this account. Kelly currently reads **3.0** against a 1.25 risk, so the tune wants to travel 1.75pp and the step is the only thing pacing it: **four days to 2.4x size**. The concurrency cliff arrives on **day two** — at 2.25% only `floor(6/2.25) = 2` full-size positions fit `maxAggregateOpenRiskPct` 6, below the configured 3, and that is journaled (`auto_tune_concurrency_reduced`) but NOT blocked | Halves the pace in both directions. Reaching Kelly takes 7 days instead of 4, and the cliff moves from day 2 to day 4 — two extra days, including a weekend, to see it and react | **The argument is proportionality, not fear.** Because the step is percentage POINTS, it is largest in relative terms exactly when risk is LOWEST: at 1.25% a 0.5pp step is a **+40% position size change in one day**; at 3.0% the same setting is +17%. The 2026-08-09 history is the mirror — at 0.24% risk the 0.5 step exceeded the entire remaining budget and auto-tune set riskPerTradePct to **0**, stopping the book (see task #47). **The cost is real and accepted**: if the new live conviction floor lifts expectancy and the out-of-sample guard confirms, this reaches Kelly three days later than it would have. **Not lower than 0.25** — at 0.1 the tune needs 18 days to answer a genuine edge change in either direction, which stops it being a tune. **NOT YET VERIFIED AT THE CONSUMER**, and there is a precise check: the daily `auto_tune_risk_increase_blocked` event carries `wouldRaiseTo`, which has read **1.75** on 09-04/05/06. The next run (2026-09-07 00:00 ET) must read **1.5**. If it still says 1.75, the clamp is not reading this field |
| 2026-09-06 | New `liveMinSignalScore` **0 → 72** (live equity only; `minSignalScore` stays **60** so paper is unchanged) | Operator decision, after the concentration read ("it keeps picking the same names") led to the score distribution | The screener's ranking predicts outcome: Spearman rho(entryScore, realized R) = **+0.320** across the 57 closed live trades carrying a score, past the 5% line at that n. Thirds: **56-69** n=19 meanR -0.073 -$59.08; **70-75** n=19 meanR -0.055 -$153.18; **76-94** n=19 meanR **+0.501 +$411.26**. Every dollar came from the top third and the bottom two thirds lost **$247.17** between them. Floor sweep over the same trades: 60 keeps 56 for +$204.81, 72 keeps 29 for +$427.22, 74 keeps 22 for +$446.17 | Better on BOTH axes, which is why it is worth the lost trades: 22 x 0.510R beats 56 x 0.128R on fewer entries and less capital at risk. It also stops the crowd-out — **17 of the 35 sub-74 entries held one of the three slots while the book was AT its cap**, blocking the higher-scoring signals behind them, so the sweep UNDERSTATES the gain by dropping those trades without adding their replacements | **72, not 74, because the number is fitted in-sample** — 72/74/76/78 all score within noise on n=57, so the sweep identifies the low 70s as a REGION and 72 concedes least if the true line sits lower. **A separate field from `minSignalScore` on purpose**: that one feeds decide.ts for BOTH books (loop.ts:602), so raising it would starve the paper control group that the stagnation counterfactual, component attribution and the exit-geometry read all depend on. Paper keeps screening at 60 and takes every signal; live takes only what clears 72, which is what keeps the comparison honest. Composes with `finishLineMinSignalScore` (65) rather than gating twice — the stricter bar binds, and at today's settings that means the floor overrules the armed-day ramp on an armed day, which two independent gates would not have done. Ships at 0 and was set through the route: a default that changes what a live book trades on deploy is a decision taken by a diff. **NOT VERIFIED AT THE CONSUMER YET** — the config write was re-read from production (only this field moved), but the live entry path runs only in market hours, so the behavioural proof is the first `live_score_floor_skipped` in Tuesday's journal. Expect noticeably fewer live entries: roughly half the recent candidates scored under 72 |
| 2026-09-06 | `regimeSizeCutPct` 0 → **40** | Operator decision, after the two-week evidence review flagged the rule as armed-but-hollow | The threshold has been `regimeAtrThresholdPct` 3 with a **0% cut** — so on the day it fires it changes nothing. Market ATR across **94 recorded entries** (57 live, 37 paper) runs **0.819%–1.230%, median 0.846%**, i.e. the trigger sits ~2.4x above anything this system has ever traded through | Turns the 3–4% ATR band from binary into graded. `maxMarketAtrPct` 4 already halts entries above 4%, so today the book takes FULL size at 3.9% ATR and nothing at 4.1%; a 40% cut is the missing middle | **40 and not 50 because these factors multiply.** `stepDownSizeCutPct` is already 50 after 2 losses, so a 3%+ ATR day following two losses now sizes at 0.5 x 0.6 = **30%** of normal; at 50/50 it would be 25%, small enough for the 1-share floor to start eating the trade. Cannot fire in any market observed, so the downside of being wrong is ~nil. Verified at the CONSUMER, not the 200: re-read from production and `POST /risk-check` reports `regime_sizing: inactive — market ATR 0.8% (triggers above 3%)`, confirming today's sizing is untouched |
| 2026-09-06 | `liveOptionsMaxOrderUsd` 1338 → **300** | Operator decision; the premise of the 2026-08-27 row above expired | That row left it at 1338 **because "live options is off"** — `liveOptionsEnabled` was re-armed **2026-09-04**, so the stated reason no longer holds. The largest order the sizer can produce is **$92.72**: 1 contract (`optionsMaxConcurrentPositions` 1) at the $0.927/share premium ceiling that falls out of `riskPerTradePct` 1.25% x $5,192 equity / the 70% `optionsDisasterStopPct`. During probation (multiplier 0.5, 10 trades left) it is ~**$46**. So 1338 was **14x** the largest legitimate order and re-anchoring would have made it $3,871 — **42x** | A backstop that can actually backstop: ~**3.2x** the largest legitimate order, still catching a sizing bug that tried to buy 4+ contracts | **This cap had never gated anything until today.** Before PR #507 no live options order could complete at all — every sub-$3 price was off Webull's nickel grid and rejected — so Tuesday 2026-09-08 is the first session where this field is load-bearing. It stays **hand-edited and frozen out of `liveCapsReanchor`**, which is correct rather than a bug: the anchor formula derives the options cap as EQUAL to the equity cap, an equity-shaped number on a premium-shaped book. Because nothing will move it automatically, the revisit trigger is written down instead: **re-derive when equity reaches ~$9k** (premium ceiling ~$1.61/share ⇒ ~$161/contract, at which 300 falls under 2x), or on any change to `riskPerTradePct`, `optionsDisasterStopPct`, or the probation multiplier. **Not** on a run of winning trades — the cap does not size anything, and raising it would only widen a backstop that is not binding; what a track record earns is the end of probation (already automatic at 10 trades) and, if wanted, `optionsMaxConcurrentPositions` or `riskPerTradePct`, after which this cap follows mechanically |
| 2026-09-09 | New `optionsAffordabilityFilterEnabled` (**false**) and `optionsAtmPremiumRatioPct` (**1.0**) — drop options candidates whose ATM contract the per-order risk budget cannot buy, before the single options slot is spent. **Ships OFF; no behaviour changes on merge** | Task #59, and F2 — the funnel is starved by the premium ceiling, and F2 forbids answering that by loosening risk | 2026-09-09 during market hours: **141 `options_blocked` vs 2 `options_passed`**. Ceiling **$0.9174/share** = (equity $5,137.44 x 1.25%) / 70% `optionsDisasterStopPct`. Measured ATM premiums the same session: RIOT $0.38 on $21.91, SMCI $0.60 on $39.08, HOOD $1.65 on $116.46, COIN $2.64 on $177.99, META $3.08 on $650.81 — **perfect separation on underlying price**, and the last three could not have filled at any score | `optionsMaxConcurrentPositions` is **1**, so this is not merely saved work: it stops the day's only options slot being spent on an arithmetically certain refusal, leaving it for a name that can fill | **The ceiling is DERIVED, never written down.** It is an exact inversion of the `quantity` rule in `optionsRiskCheck` (`premium <= maxRiskDollars / (f x 100)`), so it re-derives itself whenever equity, `riskPerTradePct` or `optionsDisasterStopPct` moves — at $25k equity COIN comes into range with no config change. The test calls the REAL `computeRiskSizing` rather than a copy of the arithmetic. **Two deliberate conservative choices**, both so the filter can only over-KEEP: (1) it reasons about a risk-% UPPER BOUND, because `methodWeightingEnabled` is on and the method lean scales UP to `expectancyMaxMultiplier` 1.5 — treating the configured 1.25% as a bound would have dropped affordable names, a bug caught only by deriving it; (2) the 1.0% assumed premium ratio sits UNDER every single-name observation (1.42%-1.73%), so it under-states contract cost. A kept symbol still meets the real risk check. **n=5, one session** — this ratio is an observation, not a law, which is why it is config. **Not verified at the consumer in production**: it ships off, so the proof is the first `options_underlying_unaffordable` event after it is enabled |
| 2026-09-09 | `short_dated_options_exit` now journals `peakGainPct` and `peakPremium`. **No parameter changed** | Rule L5 was unevaluable at its own sample threshold | The 15th closed trade landed 2026-09-09, meeting L5's minimum — and L5 could not then be answered, because the exit event carried `premiumGainPct` (gain AT exit) but never the PEAK. `give_back` fired on 4 of 15 exits (26.7%) with no way to tell a fading winner that passed the target from one that never reached it | L5 becomes answerable from the events alone: a `give_back` whose peak never reached `optionsTakeProfitPct` is not evidence of a tight trail, and one that passed it is | **Computed all along, never consumed** — the same class as the 2026-08-27 audit. The ladder derived the peak internally and even printed it in the give-back reason string ("peaked +62%, now +25%"), while the machine-readable event omitted it. `peakGainPct` is now derived ONCE in `evaluateShortDatedExit` and read by both the give-back rule and the journal, so they cannot disagree. Journaled in BOTH books. **The existing 15 trades stay unevaluable** — this is not backfillable, so L5's sample starts again from 0 |
| 2026-09-10 | `repeatEntrySizeCutPct` 0 → **40** (live only; paper opts out by written decision and stays the control arm) | Operator instruction after the 2026-09-09 loss; the percentage was the one decision #49 left open | The 09-09 live loss (−$127.10) was IRD ×2 (−$138.70): a +32% opening spike bought at 09:37, stopped in 2 minutes, then **re-entered full-size at 11:12** into a name already off its high (−$74.48). Since the knob shipped (09-08, 2 sessions): repeats n=8 **−$86.79, mean −$10.85, avgR −0.19** vs firsts +$13.15; all-time repeats n=29 avgR −0.11 vs firsts n=78 +0.06 | Repeats trade at 60% size. On 09-09 that alone saves ~$30 of the IRD re-entry and ~$35 across the day's 5 repeats, while keeping LVWR-class second setups (−0.98R then +1.93R) in the book at reduced size | **40 and not 50 for the same reason as the regime cut on 2026-09-06: these factors multiply.** `stepDownSizeCutPct` is 50 after 2 losses, so a repeat on a losing-streak day sizes at 0.5 × 0.6 = **30%** of normal — the floor that row already accepted; at 50/50 it would be 25%, where the 1-share floor starts eating the trade (DELL is already a 1-share position). Inside the playbook's 25–50% band, and the playbook's own reading — "repeats are worth less, not worthless" (86% of the all-time deficit is one DELL trade) — argues against a block. **Not verified at the consumer yet**: the proof is the first live repeat entry whose `risk_check` sizing line reads `repeat_entry: active … 40% cut` and whose quantity is 60% of what a first entry would size. Re-read at ~60 live repeats against paper's full-size control arm, per #49 |
| 2026-09-10 | `symbolReentryCooldownMinutes` 90 → **120** (live only — the cooldown itself is live-only; it reads BOTH books' closed positions, so a paper exit in a name also holds the live book off it) | Operator instruction, the lever offered from the IRD read | Since the cooldown shipped (2026-09-01, #457) no live repeat has landed inside 90 minutes — it works as configured. The window it did not cover: repeats **90–120 min** after the prior same-day exit are **n=12, −$110.21, avgR −0.09** (ORCL@118, IOT@98, DELL@97, LITE@90, **IRD@93**, GTLB@95, IOT@91, SMCI@105, FCX@92, LITE@114, CHYM@106, CHYM@91); 120–180 min n=3 −$15.27. The 09-09 IRD re-entry at 93 min (−$74.48) cleared 90 by three minutes | Blocks the 90–120 band outright; the 40% repeat cut keeps trimming what re-enters later | **Not loss-driven, so the blanket cooldown is the right shape.** In the 90–180 window, repeats after a prior LOSS ran n=7 −$49.55 (avgR −0.02) while repeats after a flat/won exit ran n=8 −$75.93 (−0.12R) — a loss-only cooldown would have kept the worse half. **n=12 and fitted near one memorable trade** — this is a small, cheap, reversible change and is recorded as such, not as a finding. The July 0–90-minute "repeats" in the raw slice (SOBR, CJMB, LVWR +1.93R…) predate the cooldown entirely and include rows whose exit timestamps precede their entries — data artefacts, excluded. **Not verified at the consumer yet**: proof is the first `Re-entry cooldown — exited Nm ago, resumes after 120m` refusal with 90 ≤ N < 120. Re-read alongside the repeat cut at ~60 live repeats |
| 2026-09-10 | New `optionsOwnExposurePool` (**false**) — the options book measures `max_sector_exposure` / `max_correlated_exposure` against its OWN open positions (premium paid) instead of a pool that folds the equity book in at full stock notional. The aggregate open-risk budget stays shared. **Ships OFF.** The operator decision recorded here is to turn it **on** for the live config once merged | Task #63 — the one live options candidate in two sessions that cleared the premium ceiling was refused on the pool | 2026-09-09 12:04 INTC: `quantity` passed (2 contracts at $0.32, $45 of risk), `max_aggregate_open_risk` passed ($238.15 + $45 vs $308.44), then **"$6,183.90 already in Information Technology vs cap $4,112.54 (80% of equity)"** — every dollar of it AMD + LITE stock. The check is bare (the pool is measured before the candidate is added) and each equity position is ~50% of equity at the 2.5% stop cap, so two same-sector stocks (~120%) close the sector to options at any size. Of the other 29 refusals 09-08/09-09, 27 were 1.9x-31x over the ceiling and 2 within 1.3x — no sizing or contract-selection change reaches them; the affordability filter (row above, live since 15:50 ET 09-09) removes them at the screen | Same shape as F8's slot split: equity runs first in the tick and fills the pool. An options entry in a sector the equity book already holds can now open when its own max loss fits the shared risk cap — INTC would have, at $283 of $308 | **Verified at the consumer in tests, both books**: shared pool refuses, own pool opens, the aggregate cap still refuses, own options positions still count against the sector cap. Risk-check journal rows now carry `exposurePool` (`shared` / `options_only`). Not yet observed in production: the proof is the first `live_options_order_placed` in a sector the equity book holds, or a `live_options_risk_blocked` row whose `exposurePool` reads `options_only` |
| 2026-09-12 | `liveOptionsMaxOrderUsd` gets **its own derivation** — `maxAffordablePremiumPerShare × 100 × 1.5`, the options sizer's own ceiling — instead of copying the equity per-order cap. The re-anchor threshold drops **15% → 5%**, and a reading more than **25% below the anchor** now HOLDS every cap for one session (journaling `equity_read_suspect`) rather than re-anchoring down | The 2026-09-06 row's own written revisit trigger ("on any change to `riskPerTradePct`, `optionsDisasterStopPct`, or the probation multiplier") plus Decision 10 of the 3%-goal plan, which asks that everything scale with the account | The old formula assigned the EQUITY cap to the options twin: a share-sized number guarding a premium-sized order. On 2026-09-06 that read **$4,269 against a $92.72 largest legitimate order — 46×** — so it was hand-set to $300 and has been frozen out of every re-anchor since. Separately, at a 15% threshold the account ran **$5.1k → $3.5k → back** with no re-anchor at all, and the one re-anchor that did fire (2026-09-11, twice) fired off a **hand-trading** reading that cut every cap ~30% while the strategy's book had not lost a cent | The options cap tracks the account like every other cap and never needs re-typing: at $5.1k / 2.5% / 70% / method lean 1.25 it derives ≈ **$273**, and it moves with equity, the risk %, and the disaster stop. Caps follow equity in days, not weeks. A bad afternoon's reading costs one session of staleness instead of a week of wrong caps | **The plan's own formula said `optionsMaxConcurrentPositions × ceiling × 100 × 1.5` (≈$550 at two slots); the slot multiplier was dropped deliberately.** `maxOrderUsd` is enforced **per order** (`order_notional` in `guardrails.ts`), so a slot count belongs in an aggregate cap; multiplying by it would leave a single order able to carry twice what the sizer can produce — a weaker backstop, not a safer one. The ceiling uses `riskPctUpperBound` (risk × `expectancyMaxMultiplier`), not the bare risk %, because the method factor is allowed to size UP and a cap below the sizer's own output is the 2026-08-27 "a correct order cannot fit its own cap" bug. The suspect-read hold is DOWNWARD only, is its own constant rather than a re-use of `equitySyncMaxJumpPct` (that field governs tick-to-tick, and loosening it to accept a deposit must not loosen this), and re-anchors as soon as the low reading survives into a second session |
| 2026-09-12 (applied) | `optionsMaxConcurrentPositions` 1 → **2**, `liveOptionsMaxOrdersPerDay` 4 → **6**, `liveOptionsMaxOrderUsd` 300 → **236** (the derived value at the $3,522.81 anchor) | The 3%-goal plan's Step 2, whose one precondition — PR A deployed — was met the same evening | PR A (#568) shipped the bid-priced, chased, tick-clamped close and the same-evening expiry sweep; the stranded HOOD 116C was settled at $0 at 02:33Z with `live_options_expired_worthless`, confirming the sweep. The first edge-leak scan puts the sleeve's gap in numbers: live options **−0.71R over 3 trades (−$171)** against paper options **+0.23R over 20** — a live-only loss, which the scan's own logic calls execution rather than decision | Two slots and six orders a day, with a per-order cap that is now **anchor-owned** for the first time since 2026-09-06: `capsCoherence` reads stored 236 / derived 236, so it follows equity from here instead of needing a re-type. Premium ceiling ≈ $1.57/share at the new risk | **No live options trade has yet run under the fixed code**, so the first one is the real test of PR A, not this row. Probation (half size) still runs to its end. The revisit trigger from the 2026-09-06 row is retired: the cap is derived again, so `riskPerTradePct`, `optionsDisasterStopPct` and equity all move it automatically |
| 2026-09-17 | **The paper book prices its exits the way live prices an order. No parameter changed.** Every paper close now goes through the same `sellableExitLimit` / `sellableSpreadExitLimit` and the same OPRA-first quote resolver as the live book (moved to `optionsExitPricing.ts`, one definition, two callers). A rule-driven exit is **decided** on tick N at the sellable price s_N and **filled** on tick N+1 at `min(s_N, s_N+1)` — what a resting sell limit chased downward does; clock-driven exits (`hard_time`, `stagnation`, the DTE time-exit) fill on their own tick. Rows record `exit_decided_at`, `exit_decision_mark` and `exit_fill_basis`; the journal gains `options_paper_exit_decided` and the close rows carry `decisionMark` / `restingLimit` / `fillPrice` / `fillBasis` / `filledOn` | The 2026-09-16 paper-book read: the sleeve's evidence was measured on fills nobody could have had | The paper book filled at the chain's midpoint, instantly, on the tick a rule fired — and a rule fires on the tick the mark is at an extreme, by construction. **HOOD 260911C116 on 2026-09-11**: paper booked +72% in 3 minutes at a 1.48 mark while the live order at 1.40 against that same mark never filled and the contract expired worthless. Over the 26 paper trades on the book: **+16.4%** of premium per trade at the mark, **+10.6%** at the sellable price (mark × 0.95, the live buffer), **+8.5%** without the HOOD fill. Six of the ten paper take-profits are ≥ +60% inside 90 minutes. The 2026-09-12 row that kept `optionsTakeProfitPct` at 60 (and the 40/30 alternatives it scored at ≈ +13% / +9%) and the 2026-09-12 (applied) row that opened the second live slot both rest on the +16.4% figure — and the alternatives were scored the same optimistic way, so the ranking may not even change; nobody knows until it is re-scored | The control arm stops overstating the sleeve. Expect the paper %-of-premium mean to fall by roughly a third to a half from here, and the take-profit rule's fills to land below the level that fired them. Nothing live changes | **Two series, not one.** Rows closed before this date filled at the mark and carry a null `exit_fill_basis`; rows after carry the basis. Any read that spans the boundary — the sleeve's mean %, L5, the 60/40/30 comparison — must say which series it is on, and the pre-committed re-score of the take-profit level runs on the new series only, once it holds ~20 trades. **A restart forgets a pending decision**, which is deliberate: the rule re-fires on the next tick and fills one tick later than it would have; a column would have added a row to keep in step with the loop for a state the loop re-derives in a minute. Partial (scale-out) slices fill at the sellable price on their own tick without the one-tick wait — a slice, not a decision; the residual optimism is one tick of spike on a fraction of a position, on a rule that has fired 0 times on this book. **Not yet verified at the consumer in production**: the proof is the first `options_paper_exit_decided` followed one tick later by a close row whose `fillPrice` ≤ `restingLimit` ≤ `decisionMark × 0.95` — and, on the first take-profit that fires on a spike, a `fillPrice` well under the `decisionMark` |
| 2026-09-18 | **Both books price an options ENTRY from the real-time ask. No parameter changed.** The live entry resolves each leg through the same OPRA-first resolver the exits use (`resolveContractQuote`, keyed by the contract symbol the signal carries) and builds its limit from the **ask** plus the 5% buffer (`buyableEntryLimit`, spread twin long ask − short bid), sizes and references at the ask, and falls back to the buffered mark only when no two-sided quote exists; the paper entry fills at the same ask | The operator's question of 2026-09-17: the account's OPRA entitlement fed the exits and not the entries | Both books re-fetched the ~15-minute-delayed chain at placement and built the buy limit from its midpoint + 5%, and the sizer's premium — the risk — was that stale midpoint; on a 0DTE that is a limit resting under a market that has left, and a contract count off by the move | Live entries fill on the price a buyer pays and are sized on it; paper entries lose the midpoint optimism the exits lost on 09-17, so expect the paper %-of-premium mean to fall a little further. Nothing else changes | **Series note**: paper rows opened before this date filled at the mark; `options_paper_order_placed` carries `fillBasis` / `quoteSource` from here, and `live_options_order_placed` carries `priceBasis` / `quoteSource` / `quoteAgeMs` / each leg's quote. Pre-committed check: the first live placement with `priceBasis 'ask'` and `quoteSource 'opra'`, its `limitPrice` the ask + 5% on the grid; a `'chain'` source on a session where the OPRA route answers is the finding to chase |
| 2026-09-18 (second) | **Each live sleeve's daily order cap counts its own opening orders, and the options sleeve's exposure ceiling is the equity sleeve's `liveMaxExposurePct` × equity (one helper, two callers). No parameter changed** | The operator's question at 11:28 ET: why no more live positions were opening | `countTodaysOrders()` counted every opening intent on the account: 4 stock entries + 2 options entries read **"6 placed vs 6/day"** against `liveOptionsMaxOrdersPerDay` from 10:27 ET, and 26 options signals had been refused by 11:36 with two placements on the sleeve's book. `buildLiveOptionsTradingConfig` pinned `maxExposureUsd` at 100% of equity while the equity sleeve's was 190%, so from 10:01 to 10:23 every options entry read **"$55,716 vs cap $29,285"** (11 refusals) | The sleeve's six orders and two slots are available on days the stock book is full of entries, as the 2026-09-12 (applied) row intended. What still refused it: the SHARED aggregate open-risk budget (5 refusals 10:01–10:10), by design — widened the same day, next row | **Not a P&L finding today**: the paper control took MU three times on the refused signals (one flat at 4.55, two closed at 2.95 from 4.60 and 4.40). Pre-committed check: the first `live_options_order_placed` on a session with four stock entries already on the book, and no `max_orders_per_day` refusal on the sleeve until its own count reaches 6 |
| 2026-09-18 (applied) | `maxAggregateOpenRiskPct` 7.5 → **12.5** | Operator instruction ("ok widen it to 12.5%"), on the read in the row above | The shared budget at 2.5% risk × 3 stock slots (7.5%) is full whenever the stock book is full, so the options sleeve could open only when a stock slot was empty: 5 `max_aggregate_open_risk` refusals between 10:01 and 10:10 ET with the stock positions on | Both sleeves can run at once: 12.5% = 2.5% × the five slots (three stock, two options). Applied at 12:02 ET through the config route; effective from the next tick, so in practice from Monday's options window | **Exposure, taken knowingly.** The daily halt stays at 7.5% of the day's opening equity; with every slot filled the book can carry 12.5% of equity at risk before the halt can act, against 7.5% before. Stock stops rest at the broker and an options position's risk is its premium, so the figure is the sum of the stops, not a gap risk. Pre-committed check: the first `live_options_order_placed` while three stock positions are open, and a `max_aggregate_open_risk` refusal only when the sum of open risk plus the candidate exceeds 12.5% of equity |
| 2026-09-19 | **Both books SELECT the contract on the real-time snapshot. No parameter changed.** After the chain fetch, `optionsDecide` sends the nearest-the-money contracts of the signal's side (at most 40, the snapshot call's own batch cap) to the OPRA snapshot and overlays each fresh two-sided print's bid/ask/mark/volume/OI/delta/IV onto the chain before `entryRules` runs (`optionsSelectionQuotes.ts`); the chain stays the strike list and the fallback, the IV-rank history keeps reading the chain, and no filter level, band or weight moves | The operator's instruction of 2026-09-18 ("build it as real time entry is important"): 09-18's placements were priced live while the contract had been chosen on a 15-minute-old chain | On 2026-09-18 the delayed chain's premiums ran 40–70% away from the live print on the 0DTE names the sleeve trades (HOOD 0.57 against 0.99, IBM 0.51 against 0.95, COIN 0.55 against 1.95), so the delta band, the spread filter and `signal.premium` — the risk the check sized — were all read on numbers the order itself no longer used | The strike, the pass/fail and the sized premium come from the same print the order pays; the paper control moves with it. With no snapshot answer the decision is byte for byte the previous one | **Series boundary 2026-09-22**: contracts selected before that session were chosen on delayed data. Journal fields `selectionQuoteSource` / `rePricedContracts` / `quoteAgeMs` / `quotesRequested` on `options_signal_generated` and `no_options_signal`. Freshness is the same `freshTwoSidedPrint` rule an order applies, so selection and pricing cannot disagree. Pre-committed check: the first `options_signal_generated` of 2026-09-22 reads `selectionQuoteSource: 'opra'` with `rePricedContracts` > 0 and `quoteAgeMs` < 120000, and the session's first entry carries `quoteSource: 'opra'`; a `'chain'` selection on a session where the snapshot answers is the finding to chase |

---

## What this plan will not do

> The **daily goal** has its own pre-committed rules and decision log in the same
> shape as this plan — `docs/TUNE_FROM_TARGET.md` §6b (2026-09-07). The equity-derived
> caveat in the data-quality notes above is exactly why that sweep runs on R per
> session rather than on account value.

It will not fit the screen to one memorable trade. The 2026-08-27 SPY entry
above is the live test of that: a hand-taken index trade made the day's entire
return while the loop, blocked four separate ways, produced one losing position.
The tempting read is "automate what worked". The disciplined one is to put SPY
and QQQ where the loop can see them, change nothing else, and let a few sessions
say whether an index-level signal survives a screen built for single-name
breakouts — knowing the honest answer may be that it does not, and that the gate
which excludes it is right to.

It will not produce a tuning change from a single day, and it will not treat
"no change recommended" as a wasted read. Most days early on should end that
way. The parameters here came from a model; the point of the next few weeks is
to accumulate enough real trades to say something about them that the model
could not — and the fastest way to destroy that is to start fitting to noise on
day two.

The 2026-09-05 review is where the accumulated picture gets judged against the
question the spec could not answer: whether there is any edge here at all.
