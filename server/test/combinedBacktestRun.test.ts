...
// Same mocking boundary convention as backtestRun.test.ts/optionsBacktestRun.test.ts:
// exercise ONLY this file's own orchestration (real-estate pre-filter reuse,
// contract discovery wiring, walk-forward window splitting) — never the
...
import {
  runCombinedBacktest,
  runCombinedWalkForwardBacktest,
  CombinedBacktestConfig,
  CombinedWalkForwardConfig,
} from '../src/services/autotrading/combinedBacktest';
...
function cfg(overrides: Partial<CombinedBacktestConfig> = {}): CombinedBacktestConfig {
  return {
    symbols: ['OK1'],
    from: '2024-01-01',
    to: '2024-01-02',
...
describe('runCombinedBacktest', () => {
  it('excludes a listed real-estate symbol before fetching any bars or contracts for it', async () => {
    ...
    const report = await runCombinedBacktest(cfg({ symbols: ['RE1', 'OK1'] }));
  ...
  it('excludes a sector-classified real-estate symbol before fetching any bars or contracts for it', async () => {
    ...
    const report = await runCombinedBacktest(cfg({ symbols: ['RE2', 'OK1'] }));
  ...
  it('fetches contract reference data for every eligible symbol whose equity bars actually resolved', async () => {
    ...
    await runCombinedBacktest(cfg({ symbols: ['HASDATA', 'NODATA'], from: '2024-06-01', to: '2024-06-05' }));
    ...
    expect(toExp > '2024-06-05').toBe(true); // padded by maxDaysToExpiration
  });

  it('does not crash when a symbol has no historical bars, and reports empty trade lists for it', async () => {
    const report = await runCombinedBacktest(cfg({ symbols: ['NODATA'] }));
  ...
  it("isolates one symbol's equity-bar fetch failure — the rest of the batch still runs, error reported separately", async () => {
    ...
    const report = await runCombinedBacktest(cfg({ symbols: ['BAD1', 'OK1'] }));
...
describe('runCombinedWalkForwardBacktest', () => {
  it('splits into independent in-sample/out-of-sample windows, fetching history and contracts only once', async () => {
    ...
    const wfCfg: CombinedWalkForwardConfig = {
      symbols: ['WF1'],
      from,
      to,
      splitDate,
  ...
  it('excludes real estate once, upfront, shared across both windows', async () => {
    mockIsExcluded.mockImplementation((s) => s === 'RE1');
    const result = await runCombinedWalkForwardBacktest({
