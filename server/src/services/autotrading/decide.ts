...
export interface TradeSignal {
   ...
   *  be a second, differently-timed measurement of the same quantity.
   *
   *  Deliberately NOT gated on here. This used to refuse the candidate inside
   *  generateSignal, which sits ABOVE the paper/live split — loop.ts calls
   *  decide once and both books consume the same signals — so the filter
   *  silently removed those names from PAPER too. Every other entry gate
   *  (symbolCooldown, reentryCooldown, the naked-short skip) is live-only
