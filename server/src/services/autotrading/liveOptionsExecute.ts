...
// The sell-side pricing this file used to own — the marketable buffer, the
// OPRA-first quote resolver, sellableExitLimit and its spread twin — moved to
// optionsExitPricing.ts on 2026-09-17 so the PAPER book prices its exits with
// the same functions instead of filling at the midpoint. Re-exported here so
// this module's tests and callers read as before; the definitions are not
// duplicated.
export {
...
export type { ExitPriceBasis, ResolvedExitQuote, SellableExitLimit } from './optionsExitPricing';
 ...
 * delayed chain — the shared resolver, with this book's chain fetch as the
 * fallback. See optionsExitPricing.ts.
 */
export async function resolveLiveExitQuote(
  ...
  return resolveExitQuote(contractSymbol, () => fetchContractQuote(symbol, expiration, strike, side), now);
}
